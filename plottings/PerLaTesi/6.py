...
b = filter(df,23,'D(mean_wv(F,R),mean_wv(F,S))','D(mean_wv(Vu,R),mean_wv(Vu,S))')
...
#fig.suptitle('Unconditioned and conditioned comparison',fontsize=13)
...
ax[1].scatter(df['D(mean_wv(F,R),mean_wv(F,S))'], df['D(mean_wv(Vu,R),mean_wv(Vu,S))'], c='#7d5f8d',marker='.', s=size, alpha=transparency, label='models average raw')
ax[1].scatter(b['D(mean_wv(F,R),mean_wv(F,S))'], b['D(mean_wv(Vu,R),mean_wv(Vu,S))'], c='#00D7D7',marker='.', s=size, alpha=transparency, label='filtered')
...
z = np.polyfit(df['D(mean_wv(F,R),mean_wv(F,S))'], df['D(mean_wv(Vu,R),mean_wv(Vu,S))'], 2)
p = np.poly1d(z)
ax[1].plot(xs, p(xs), "b-",linewidth=0.7, label='polynomial fit D=2')
ax[1].set_xlim([-1,1])
ax[1].set_ylim([-1,1])
