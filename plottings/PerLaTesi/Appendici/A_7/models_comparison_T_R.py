...
z = np.polyfit(df['D(llava(F,R),llava(F,S))'], df['D(llava(Vu,R),llava(Vc,R))'], 3)
p = np.poly1d(z)
...
ax[1].set_xlim([-1,1])
ax[1].set_ylim([-1,1])
z = np.polyfit(df['D(clip_flant(F,R),clip_flant(F,S))'], df['D(clip_flant(Vu,R),clip_flant(Vc,R))'], 3)
p = np.poly1d(z)
...
ax[2].set_xlim([-1,1])
ax[2].set_ylim([-1,1])
z = np.polyfit(df['D(instructblip(F,R),instructblip(F,S))'], df['D(instructblip(Vu,R),instructblip(Vc,R))'], 3)
p = np.poly1d(z)
