...
ax[0].scatter(df['D(llava(F,R),llava(F,S))'], df['D(llava(Vu,R),llava(Vc,R))'], c='#145d9e',marker='.', s=size, alpha=transparency)
ax[0].set_xlabel('llava(F,$T_R$) - llava(F,$T_S$)')
ax[0].set_ylabel('llava($V_S^U$,$T_R$) - llava($V_S^C$,$T_R$)')
ax[0].set_title('LLAVA')
ax[0].set_xlim([-1,1])
ax[0].set_ylim([-1,1])
z = np.polyfit(df['D(llava(F,R),llava(F,S))'], df['D(llava(Vu,R),llava(Vc,R))'], 4)
p = np.poly1d(z)
ax[0].scatter(df['D(llava(F,R),llava(F,S))'], p(df['D(llava(F,R),llava(F,S))']), c='#ff0000',marker='.', s=1.5, label='Trend line')


ax[1].scatter(df['D(clip_flant(F,R),clip_flant(F,S))'], df['D(clip_flant(Vu,R),clip_flant(Vc,R))'], c='#f79410',marker='.', s=size, alpha=transparency)
ax[1].set_xlabel('clip_flant(F,$T_R$) - clip_flant(F,$T_S$)')
ax[1].set_ylabel('clip_flant($V_S^U$,$T_R$) - clip_flant($V_S^C$,$T_R$)')
ax[1].set_title('clip_flant')
ax[1].set_xlim([-1,1])
ax[1].set_ylim([-1,1])
z = np.polyfit(df['D(clip_flant(F,R),clip_flant(F,S))'], df['D(clip_flant(Vu,R),clip_flant(Vc,R))'], 4)
p = np.poly1d(z)
...
ax[2].scatter(df['D(instructblip(F,R),instructblip(F,S))'], df['D(instructblip(Vu,R),instructblip(Vc,R))'], c='#0cb14d',marker='.', s=size, alpha=transparency)
ax[2].set_xlabel('instructblip(F,$T_R$) - instructblip(F,$T_S$)')
ax[2].set_ylabel('instructblip($V_S^U$,$T_R$) - instructblip($V_S^C$,$T_R$)')
ax[2].set_title('instructblip')
ax[2].set_xlim([-1,1])
ax[2].set_ylim([-1,1])
z = np.polyfit(df['D(instructblip(F,R),instructblip(F,S))'], df['D(instructblip(Vu,R),instructblip(Vc,R))'], 4)
p = np.poly1d(z)
...
plt.savefig(f'P7_models_comparison_T_R.png', dpi=300)
plt.clf()
