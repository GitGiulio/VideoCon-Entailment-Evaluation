...
a = filter(df,23,'clip_flant(F,S)','D(clip_flant(Vc_mean,R),clip_flant(Vc_mean,S))')
b = filter(df,23,'llava(F,S)','D(llava(Vc_mean,R),llava(Vc_mean,S))')
c = filter(df,23,'instructblip(F,S)','D(instructblip(Vc_mean,R),instructblip(Vc_mean,S))')
size = 1
transparency = 1  # df[y]
...
ax[0].scatter(df['clip_flant(F,S)'], df['D(clip_flant(Vc_mean,R),clip_flant(Vc_mean,S))'], c='#f79410',marker='.', s=size, alpha=transparency, label='models avereage raw')
ax[0].scatter(a['clip_flant(F,S)'], a['D(clip_flant(Vc_mean,R),clip_flant(Vc_mean,S))'], c='#e77410',marker='.', s=size, alpha=transparency, label='filtered data')
ax[0].set_xlabel('clip_flant($F,T_S$)')
ax[0].set_ylabel('clip_flant($V_S^C,T_R$) - clip_flant($V_S^C,T_S$)')
ax[0].set_title('Clip flant')
ax[0].set_xlim([0,1])
ax[0].set_ylim([-1,1])
z = np.polyfit(df['clip_flant(F,S)'], df['D(clip_flant(Vc_mean,R),clip_flant(Vc_mean,S))'], 1)
p = np.poly1d(z)
xs = np.linspace(0, 1, 10000)
ax[0].plot(xs, p(xs), "r-",linewidth=0.7, label='linear regression')


ax[1].scatter(df['llava(F,S)'], df['D(llava(Vc_mean,R),llava(Vc_mean,S))'], c='#1a6fc4',marker='.', s=size, alpha=transparency, label='models avereage raw')
ax[1].scatter(b['llava(F,S)'], b['D(llava(Vc_mean,R),llava(Vc_mean,S))'], c='#0f3fba',marker='.', s=size, alpha=transparency, label='filtered data')
ax[1].set_xlabel('llava($F,T_S$)')
ax[1].set_ylabel('llava($V_S^C,T_R$) - llava($V_S^C,T_S$)')
ax[1].set_title('Llava')
ax[1].set_xlim([0,1])
ax[1].set_ylim([-1,1])
z = np.polyfit(df['llava(F,S)'], df['D(llava(Vc_mean,R),llava(Vc_mean,S))'], 1)
p = np.poly1d(z)
ax[1].plot(xs, p(xs), "r-",linewidth=0.7, label='linear regression')

ax[2].scatter(df['instructblip(F,S)'], df['D(instructblip(Vc_mean,R),instructblip(Vc_mean,S))'], c='#0cb14d',marker='.', s=size, alpha=transparency, label='models avereage raw')
ax[2].scatter(c['instructblip(F,S)'], c['D(instructblip(Vc_mean,R),instructblip(Vc_mean,S))'], c='#0c811d',marker='.', s=size, alpha=transparency, label='filtered data')
...
ax[2].set_xlim([0,1])
ax[2].set_ylim([-1,1])
z = np.polyfit(df['instructblip(F,S)'], df['D(instructblip(Vc_mean,R),instructblip(Vc_mean,S))'], 1)
p = np.poly1d(z)
...
plt.savefig(f'D4_models_comparison_FS.png', dpi=300)
plt.clf()
