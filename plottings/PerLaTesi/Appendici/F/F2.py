...
a = filter(df,33,'D(mean_wv(F,R),mean_wv(F,S))','D(mean_wv(Vu,R),mean_wv(Vc,R))')
...
plt.scatter(df['D(mean_wv(F,R),mean_wv(F,S))'], df['D(mean_wv(Vu,R),mean_wv(Vc,R))'], c='#7d5f8d',marker='.', s=size, alpha=transparency,label='raw data')
plt.scatter(a['D(mean_wv(F,R),mean_wv(F,S))'], a['D(mean_wv(Vu,R),mean_wv(Vc,R))'], c='#00D7D7',marker='.', s=size, alpha=transparency,label='filtered')
plt.xlabel('models_average(F,$T_R$) - models_average(F,$T_S$)')
plt.ylabel('models_average($V_S^U$,$T_R$) - models_average($V_S^C$,$T_R$)')
plt.xlim([-1,1])
plt.ylim([-1,1])

xs = np.linspace(-1, 1, 10000)
ys = np.linspace(0, 0, 10000)
z = np.polyfit(a['D(mean_wv(F,R),mean_wv(F,S))'], a['D(mean_wv(Vu,R),mean_wv(Vc,R))'], 1)
p = np.poly1d(z)
plt.plot(xs, p(xs),'r-',linewidth=0.6, label='linear regression')

xs = np.linspace(0, 1, 10000)
z = np.polyfit(a['D(mean_wv(F,R),mean_wv(F,S))'], a['D(mean_wv(Vu,R),mean_wv(Vc,R))'], 2)
p = np.poly1d(z)
plt.plot(xs, p(xs),'b-',linewidth=0.6, label='polynomial fitting')
xs = np.linspace(-1, 1, 10000)
