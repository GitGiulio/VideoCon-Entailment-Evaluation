...
plt.xlim([-1,1])
plt.ylim([-1,1])
z = np.polyfit(b['D(mean_wv(F,R),mean_wv(F,S))'], b['D(mean_wv(Vu,S),mean_wv(Vc,S))'], 2)
p = np.poly1d(z)
xs = np.linspace(0, 1, 10000)

plt.plot(xs, p(xs), 'r-',linewidth=0.6, label='Trend line')

z = np.polyfit(b['D(mean_wv(F,R),mean_wv(F,S))'], b['D(mean_wv(Vu,S),mean_wv(Vc,S))'], 1)
p = np.poly1d(z)
...
print(df['D(mean_wv(Vu,S),mean_wv(Vc,S))'].shape)
print(b['D(mean_wv(Vu,S),mean_wv(Vc,S))'].shape)
from scipy.stats import pearsonr
pccdf, _ = pearsonr(df['D(mean_wv(F,R),mean_wv(F,S))'], df['D(mean_wv(Vu,S),mean_wv(Vc,S))'])
pccB, _ = pearsonr(b['D(mean_wv(F,R),mean_wv(F,S))'], b['D(mean_wv(Vu,S),mean_wv(Vc,S))'])
print(f'all: {pccdf} | B({0.0},1): {pccB}')
