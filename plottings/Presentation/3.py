...
plt.title('Unconditioned - conditioned trend', fontsize=13)

plt.scatter(df['D(mean_wv(F,R),mean_wv(F,S))'], df['D(mean_wv(Vu,S),mean_wv(Vc,S))'], c='#7d5f8d',marker='.', s=size, alpha=transparency)
plt.scatter(a['D(mean_wv(F,R),mean_wv(F,S))'], a['D(mean_wv(Vu,S),mean_wv(Vc,S))'], c='#00D7D7',marker='.', s=size, alpha=transparency)
plt.xlabel('E(F,$T_R$) - E(F,$T_S$)')
plt.ylabel('E($V_S^U$,$T_S$) - E($V_S^C$,$T_S$)')
plt.xlim([-1,1])
plt.ylim([-1,1])
...
plt.savefig('Main_pattern.png', dpi=1000)
plt.clf()
